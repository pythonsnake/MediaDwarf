...
def add_media(user, type, metadata):
    media = fixture_media_entry(uploader=user.id, save=False,
                                expunge=False, fake_upload=True)
    media.media_type = u'mediagoblin.media_types.' + type
    media.metadata = metadata

    return media

class TestOembed(object):
    def setup(self):
        # set up new user
        ...
        self.login(test_app)
        self.media_image = add_media(user_a, 'image', {u'width': 1920, u'height': 1080})
        Session.add(media_image)
        
        self.media_video = add_media(user_a, 'video', 
                                     {u'medium_size': [640, 640], u'vorbis_quality': 0.3,
                                      u'vp8_threads': 0, u'vp8_quality': 8})
        Session.add(media_video)

        self.media_audio = add_media(user_a, 'audio', None)
        Session.add(media_audio)


    def test_url(self, test_app):
        test_app.get('/oembed?url=
